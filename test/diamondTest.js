...
describe('DiamondTest', async function () {
  ...
  before(async function () {
    ...
    LIBERC1155FACET = await ethers.getContractAt('LIBERC1155FACET', diamondAddress)
    LIBMAINFACET = await ethers.getContractAt('LIBMAINFACET', diamondAddress)
  })

  it('should have three facets -- call to facetAddresses function', async () => {
    for (const address of await diamondLoupeFacet.facetAddresses()) {
      addresses.push(address)
      console.log(address , " Address of facet")
    }
  ...
  it('facets should have the right function selectors -- call to facetFunctionSelectors function', async () => {
    ...
    selectors = getSelectors(LIBERC1155FACET)
    ...
    assert.sameMembers(result, selectors)
    selectors = getSelectors(LIBMAINFACET)
    result = await diamondLoupeFacet.facetFunctionSelectors(addresses[4])
    assert.sameMembers(result, selectors)
  })

// 0x4e1273f4 0xcdffacc6 0xf2fde38b

  // it('selectors should be associated to facets correctly -- multiple calls to facetAddress function', async () => {
  //   assert.equal(
  //     addresses[1],
  //     await diamondLoupeFacet.facetAddress('0x4e1273f4')
  //   )
  //   assert.equal(
  //     addresses[2],
  ...
  // it('should add LIBERC1155FACET functions', async () => {
  //   const LIBERC1155FACET = await ethers.getContractFactory('PubgItems')
  //   const LIBERC1155FACET_INSTANCE = await LIBERC1155FACET.deploy()
  //   await LIBERC1155FACET_INSTANCE.deployed()
  //   addresses.push(LIBERC1155FACET_INSTANCE.address)
  //   const selectors = getSelectors(LIBERC1155FACET_INSTANCE)
  //   tx = await diamondCutFacet.diamondCut(
  //     [{
  //       facetAddress: LIBERC1155FACET_INSTANCE.address,
  ...
  //   }
  //   result = await diamondLoupeFacet.facetFunctionSelectors(LIBERC1155FACET_INSTANCE.address)
  //   assert.sameMembers(result, selectors)
  // })

  // it('should test function call', async () => {
  //   const accounts = await ethers.getSigners()
  //   const lIBERC1155FACET = await ethers.getContractAt('LIBERC1155FACET', diamondAddress)
  //   await lIBERC1155FACET.createCollection(2,2)
  //   // let tnx = await lIBERC1155FACET.balanceOf(accounts[0].address,1)
  //   // console.log(tnx.toString())
  // })
